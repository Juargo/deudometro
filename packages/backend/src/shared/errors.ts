export class DomainError extends Error {
  constructor(
    public readonly code: string,
    public readonly httpStatus: number,
    message?: string
  ) {
    super(message || code);
    this.name = 'DomainError';
  }
}

// Error codes
export const USER_ALREADY_EXISTS = 'USER_ALREADY_EXISTS';
export const REGISTRATION_FAILED = 'REGISTRATION_FAILED';
export const INVALID_TOKEN = 'INVALID_TOKEN';
export const TOKEN_EXPIRED = 'TOKEN_EXPIRED';
export const MEMBER_ALREADY_EXISTS = 'MEMBER_ALREADY_EXISTS';
export const INVITATION_NOT_FOUND = 'INVITATION_NOT_FOUND';
export const INVITATION_EXPIRED = 'INVITATION_EXPIRED';
export const INVITATION_USED = 'INVITATION_USED';
export const PROFILE_NOT_FOUND = 'PROFILE_NOT_FOUND';
export const FORBIDDEN = 'FORBIDDEN';
export const INVALID_CREDENTIALS = 'INVALID_CREDENTIALS';
export const EMAIL_ALREADY_REGISTERED = 'EMAIL_ALREADY_REGISTERED';

// M2 — Profile & Debt errors
export const DEBT_NOT_FOUND = 'DEBT_NOT_FOUND';
export const DEBT_ALREADY_ARCHIVED = 'DEBT_ALREADY_ARCHIVED';
export const DEBT_NOT_IN_SPACE = 'DEBT_NOT_IN_SPACE';
export const DEBT_NOT_VISIBLE = 'DEBT_NOT_VISIBLE';
export const INSUFFICIENT_INCOME_OR_CAPITAL = 'INSUFFICIENT_INCOME_OR_CAPITAL';
export const INVALID_INTEREST_RATE = 'INVALID_INTEREST_RATE';
export const INVALID_MONTHLY_ALLOCATION = 'INVALID_MONTHLY_ALLOCATION';
export const ORIGINAL_BALANCE_IMMUTABLE = 'ORIGINAL_BALANCE_IMMUTABLE';
export const SHARING_OWNER_ONLY = 'SHARING_OWNER_ONLY';
export const INVALID_FIXED_EXPENSES = 'INVALID_FIXED_EXPENSES';

// M4 — Payment tracking errors
export const DEBT_ALREADY_PAID = 'DEBT_ALREADY_PAID';
export const PAYMENT_EXCEEDS_BALANCE = 'PAYMENT_EXCEEDS_BALANCE';
export const DUPLICATE_PAYMENT = 'DUPLICATE_PAYMENT';
export const PAYMENT_NOT_FOUND = 'PAYMENT_NOT_FOUND';
export const INVALID_PAYMENT_AMOUNT = 'INVALID_PAYMENT_AMOUNT';
export const MILESTONE_NOT_FOUND = 'MILESTONE_NOT_FOUND';

// M3 — Plan generation errors
export const NO_SURPLUS = 'NO_SURPLUS';
export const INSUFFICIENT_BUDGET = 'INSUFFICIENT_BUDGET';
export const NO_ACTIVE_DEBTS = 'NO_ACTIVE_DEBTS';
export const PLAN_NOT_FOUND = 'PLAN_NOT_FOUND';
export const PLAN_ALREADY_HAS_AI = 'PLAN_ALREADY_HAS_AI';
export const AI_TIMEOUT = 'AI_TIMEOUT';
export const AI_CIRCUIT_OPEN = 'AI_CIRCUIT_OPEN';
export const AI_RESPONSE_INVALID = 'AI_RESPONSE_INVALID';
export const PLAN_GENERATION_FAILED = 'PLAN_GENERATION_FAILED';
export const MAX_MONTHS_EXCEEDED = 'MAX_MONTHS_EXCEEDED';
export const NO_ACTIVE_PLAN = 'NO_ACTIVE_PLAN';

// M8 — Financial diagnosis errors
export const DIAGNOSIS_INTENTION_REQUIRED = 'DIAGNOSIS_INTENTION_REQUIRED';
export const DIAGNOSIS_GENERATION_FAILED = 'DIAGNOSIS_GENERATION_FAILED';
export const DIAGNOSIS_INTENTION_TOO_LONG = 'DIAGNOSIS_INTENTION_TOO_LONG';
export const INVALID_EMPLOYMENT_STATUS = 'INVALID_EMPLOYMENT_STATUS';
export const INVALID_INVESTMENT_KNOWLEDGE = 'INVALID_INVESTMENT_KNOWLEDGE';
